...
if st.button("🔗 Join Files to HCP-Date Base"):
    ...
    for fname, config in file_join_configs.items():
        ...
        try:
            ...
            df = df.rename({hcp_col: "HCP_ID", date_col: "Date",zip_col : "ZIP"})
            if col_renames:
...
if st.button("🔗 Join Files to DTC-Date Base"):
    ...
    # Store in session state
    final_df = final_df.fill_null(0)
    st.session_state["geo_column"] = dtc_col
...
if (final_hcp_data is not None
    and final_dtc_data is not None
    and not final_hcp_data.is_empty()
    and not final_dtc_data.is_empty()
    and zip_dma_file is not None  ):

    zip_dma_df = pl.read_csv(zip_dma_file)
    st.dataframe(zip_dma_df)
    # Ensure column names are consistent
    # for col in ['zip', 'dma_id', 'date']:
    #     if col not in final_hcp_data.columns and col in zip_dma_df.columns:
    #         st.error(f"Column '{col}' missing in HCP or ZIP-DMA data.")
    #     if col not in final_dtc_data.columns and col in zip_dma_df.columns:
    #         st.error(f"Column '{col}' missing in DTC or ZIP-DMA data.")

    # First join: HCP + ZIP-DMA on 'zip'
    hcp_dma_df = final_hcp_data.join(zip_dma_df, left_on="ZIP",right_on="ZIP_CODE" ,how="left")

    # Second join: HCP+DMA with DTC on 'dma_id' and 'date'
    final_df = hcp_dma_df.join(final_dtc_data, left_on=["DMA_code", "Date"],right_on=["DMA_CODE", "Date"], how="left")
    ...
    st.info("Please upload all three required CSV files.")